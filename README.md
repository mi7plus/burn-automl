...
| `automl-burn` | Burn deep-learning execution adapter: train Burn models as `automl-core` objectives with per-epoch reporting and pruning; includes the high-level `AutoClassifier`/`AutoRegressor` tabular API. CPU (`ndarray`) backend by default. |
