...
use automl_core::trial::TrialRecord;
...
/// Deterministic train/validation split of `n` rows: the last `val_fraction`
/// (by shuffled order) become validation.
...
impl AutoClassifier {
    /// A new builder with default schedule (5 epochs, batch 32, 20 trials).
    pub fn new() -> Self {
        AutoClassifier {
            features: Vec::new(),
            labels: Vec::new(),
    ...
    pub fn val_fraction(mut self, f: f64) -> Self {
        self.val_fraction = f.clamp(0.05, 0.9);
        self
    }
    ...
    pub fn fit(self) -> Result<AutoResult> {
        ...
        let (train_idx, val_idx) = split(self.features.len(), self.val_fraction, self.seed);
        let data = Arc::new(gather(&self.features, &self.labels, &train_idx, &val_idx));
        ...
        let objective = move |p: &ParamSet, sink: &mut dyn ReportSink| -> Result<NamedMetrics> {
            ...
            let acc = train_classifier::<TrainBackend>(&cfg, &data, sink);
...
impl AutoRegressor {
    ...
    pub fn fit(self) -> Result<AutoResult> {
        ...
        let feat_dim = self.features[0].len();
        let labels: Vec<i64> = vec![0; self.features.len()]; // unused placeholder
        let (train_idx, val_idx) = split(self.features.len(), self.val_fraction, self.seed);
        let data = Arc::new(gather_reg(
            &self.features,
            &self.targets,
            &train_idx,
            &val_idx,
        ...
        let objective = move |p: &ParamSet, sink: &mut dyn ReportSink| -> Result<NamedMetrics> {
            ...
            let rmse = train_regressor::<TrainBackend>(&cfg, &data, sink);
